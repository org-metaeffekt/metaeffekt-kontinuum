...
#Run maven command
run_maven_command() {
  CMD=(mvn -f "$PROCESSORS_DIR/util/util_enrich-with-reference.xml" process-resources)
  [ "${DEBUG:-}" = "true" ] && CMD+=("-X")
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_debug "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/util/util_enrich-with-reference.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/util/util_enrich-with-reference.xml because the maven execution was unsuccessful"
      return 1
  fi
}
