...
run_maven_command() {
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_mvn "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/analyze/analyze_resolve-inventory.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/analyze/analyze_resolve-inventory.xml because the maven execution was unsuccessful"
      return 1
  fi
}
