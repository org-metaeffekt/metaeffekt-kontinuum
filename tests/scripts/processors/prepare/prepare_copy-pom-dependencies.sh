...
CASE="prepare/prepare_copy-pom-dependencies-01.sh"
...
run_maven_command() {
  ...
  log_info "Running processor $PROCESSORS_DIR/prepare/prepare_copy-pom-dependencies.xml"
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_mvn "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/prepare/prepare_copy-pom-dependencies.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/prepare/prepare_copy-pom-dependencies.xml because the maven execution was unsuccessful"
      return 1
  fi
}
