...
CASE="prepare/prepare_save-inspect-image-01.sh"
...
#Run maven command
run_maven_command() {
  CMD=(mvn -f "$PROCESSORS_DIR/prepare/prepare_save-inspect-image.xml" process-resources)
  ...
  log_info "Running processor $PROCESSORS_DIR/prepare/prepare_save-inspect-image.xml"
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_mvn "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/prepare/prepare_save-inspect-image.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/prepare/prepare_save-inspect-image.xml because the maven execution was unsuccessful"
      return 1
  fi
}
