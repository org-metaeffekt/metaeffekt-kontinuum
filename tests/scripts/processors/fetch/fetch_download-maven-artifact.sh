...
CASE="fetch/fetch_download-maven-artifact-01.sh"
...
# Run maven command
run_maven_command() {
  CMD=(mvn -f "$PROCESSORS_DIR/fetch/fetch_download-maven-artifact.xml" process-resources)
  [ "${DEBUG:-}" = "true" ] && CMD+=("-X")
  ...
  CMD+=("-Doutput.asset.dir=$OUTPUT_ASSET_DIR")
