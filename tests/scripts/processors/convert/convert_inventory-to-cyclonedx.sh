...
run_maven_command() {
  ...
  log_info "Running processor $PROCESSORS_DIR/convert/convert_inventory-to-cyclonedx.xml"
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_mvn "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/convert/convert_inventory-to-cyclonedx.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/convert/convert_inventory-to-cyclonedx.xml because the maven execution was unsuccessful"
      return 1
  fi
}
