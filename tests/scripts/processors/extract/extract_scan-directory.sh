...
run_maven_command() {
  ...
  log_info "Running processor $PROCESSORS_DIR/extract/extract_scan-directory.xml"

  log_config "input.reference.inventory.dir=$INPUT_REFERENCE_INVENTORY_DIR
              input.extract.dir=$INPUT_EXTRACT_DIR" "
  ...
  if "${CMD[@]}" 2>&1 | while IFS= read -r line; do log_mvn "$line"; done; then
      log_info "Successfully ran $PROCESSORS_DIR/extract/extract_scan-directory.xml"
  else
      log_error "Failed to run $PROCESSORS_DIR/extract/extract_scan-directory.xml because the maven execution was unsuccessful"
      return 1
  fi
}
